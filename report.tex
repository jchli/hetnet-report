...
I found \href{http://stackoverflow.com/questions/9046572/how-to-understand-the-directory-structure-of-android-root-tree}{this} Stackoverflow post very useful in understanding the purpose of the various directories in the Android repository. The following directories are most relevant to development:
\begin{itemize}
...
Above shows a diagram of our implementation design. Gray rectangles are classes, while yellow ones are methods. Entities marked red are ones being modified. We first enhance \verb|NetworkCapabilities| with the location parameter. Then, in \verb|NetworkAgent|, we implement an additional method called \verb|getScoreForRequest|, which matches network requests with the capabilities of the network, and calculates a score for the network based on its distance to the requested location. In \verb|ConnecitivtyService|, instead of scoring networks on their characteristics alone without considering network requests, we use the new \verb|getScoreForRequest| function to score networks based on the extent to which they fulfill the given request. Thus the highest-ranked network will be the one nearest to the requested location, and Android will switch to that network. \\
...
\end{verbatim}
The cause of this appears to be a security feature in Android, which randomizes the address space of processes, making it difficult for attackers to exploit bugs in the program. There was not enough time to investigate how to properly cross-compile HIP as a position-independent executable and load it onto Android.

\subsection{Implementation}
We show here some code snippets in my implementation, and explain their purpose. \\
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.25]{figures/location.png}
  \caption{Enhancing NetworkCapabilities with Location}
\end{figure}
We add a location field to \verb|NetworkCapabilities|. The \verb|satisfiedByLocation| function checks if two networks are close to each other (i.e. within some constant radius). Recall that when matching networks and requests, ConnectivityService first filters all networks with a boolean condition before proceeding to rank them on their score. This \verb|satisfiedByLocation| function serves as part of that boolean condition, i.e. if a network is too far away from the requested location (or if it simply does not have a location), it is filtered out and no longer considered. \\
The constant 60 was chosen arbitrarily and has not been verified. It may make sense to modfiy its value, parameterize it on another statistic (e.g. signal strength), or remove it altogether. Further investigation is needed to make sure the condition works well with real networks. \\
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.25]{figures/getScoreForRequest.png}
  \caption{getScoreForRequest}
\end{figure}
With the location field added to the capabilities vector, we now proceed to evaluate network scores based on their location. Figure 4.2 shows the \verb|getScoreForRequest| function in \verb|NetworkAgentInfo|. Given a particular network request, we score a network on its distance to the requested location. If it is far away from the requested location, we apply a penalty to the network score, otherwise we keep its score unchanged (recall that scores are constantly being updated by bearer-specific code, so \verb|getScoreRequest| should not invent its own score, but only build on the current score). \\

Figure 4.3 shows the use of \verb|getScoreForRequest| in \verb|ConnectivityService|. In place of the original \verb|getCurrentScore| method, we are using the new method that takes network requests into account when ranking networks. If a network scores higher than another with regard to a particular request, it is then accepted for that request.
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.25]{figures/rematch.png}
  \caption{Ranking Networks by Request-based Scores}
\end{figure}
Finally, with the scoring mechanism in place, we demonstrate its functionality by requesting a network that is physically close to the device. \verb|ConnectivityService| maintains a default request, which is what dominates the network choice in usual circumstances. Thus we request a nearby network by setting default request's location field to the current location. Figure 4.4 shows how we dynamically listen to location updates from \verb|LocationManager| and update the location of the default request. This ensures that we are always requesting for the network closest to the device.
\begin{figure}[H]
  \centering
...
The above code segments show the core of the implementation effort in Android. The demo is able to run on Android emulator, but since we don't have access to a device with both WiFi and cellular networks, the network switch has not been tested. 
...
\item Enhance networks objects in Android with more statistics such as bandwidth, measured and stored after a link is established. Another possible approach is to develop a prototype for the MIIS database and query that database within Android.
\item Develop a full-fledged policy engine that considers multiple parameters and makes an intelligent decision about network choice.
