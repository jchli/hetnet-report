...
As mentioned, this is the method that applications invoke to request a network with the specified \verb|NetworkCapabilities|. 
\item \verb|rematchNetworkAndRequests|
\end{enumerate}

\subsection{NetworkAgent and NetworkAgentInfo}
\verb|NetworkAgent| provides a common interface for different types of networks to report their status to \verb|ConnectivityService|. For example, it provides methods such as \verb|sendLinkProperties|, \verb|sendNetworkScore|, which gets called when there is a change in a network's properties or score. These updates are done with a mesage-passing style Its subclasses optionally extend these methods to provide code that deals specifically with one type of network. \verb|NetworkAgentInfo| is a wrapper around \verb|NetworkAgent|, and \verb|ConnectivityService| communicates directly with \verb|NetworkAgentInfo| to obtain information about all networks. 
